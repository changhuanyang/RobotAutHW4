...
class DiscreteEnvironment(object):
	...
	def __init__(self, resolution, lower_limits, upper_limits):
		...
		for idx in range(self.dimension):
			self.num_cells[idx] = np.ceil((upper_limits[idx] - lower_limits[idx])/self.resolution[idx])+1
	...
	def ConfigurationToGridCoord(self, config):
		...
		for idx in range(self.dimension):
			coord[idx] = round(config[idx] - 0.5 * self.resolution[idx], 2) - self.lower_limits[idx]
			coord[idx] = round(coord[idx]/self.resolution[idx])
		return coord
	...
	def GridCoordToConfiguration(self, coord):
		...
		config = [0] * self.dimension
		for idx in range(self.dimension):
			config[idx] = self.lower_limits[idx] + coord[idx] * self.resolution[idx] + 0.5 * self.resolution[idx]
		return config
